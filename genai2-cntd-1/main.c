// main.c - Sorting algorithms benchmark with insertion sort, bubble sort, quicksort, and library qsort
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
#include <time.h>
...
double bench_qsort(int original[], int n, int iterations);
void get_timestamp(char *timestamp, int len);
void write_results_to_csv(const char *filename, int N, int E, double quicksort_time, double qsort_time, const char *timestamp);
...
int main(int argc, char *argv[]) {
    ...
    // Benchmark library qsort
    generate_random_array(original_array, N);
    double avg_qsort = bench_qsort(original_array, N, E);
    printf("Library qsort: %.6f seconds average\n", avg_qsort);

    // Write results to CSV file
    write_results_to_csv(csv_filename, N, E, avg_quick, avg_qsort, provided_timestamp);
...
void write_results_to_csv(const char *filename, int N, int E, double quicksort_time, double qsort_time, const char *timestamp) {
    ...
    if (file_size == 0) {
        // Write header
        fprintf(file, "timestamp,N,E,quicksort_time,qsort_time\n");
    }

    // Write data with provided timestamp
    fprintf(file, "%s,%d,%d,%.6f,%.6f\n",
            timestamp, N, E, quicksort_time, qsort_time);
...
// Comparison function for qsort (from stdlib.h)
...
// Benchmark function for library qsort
double bench_qsort(int original[], int n, int iterations) {
    int *arr = (int *)malloc(n * sizeof(int));
    if (!arr) {
        return -1;
