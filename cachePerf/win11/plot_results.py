...
def run_benchmark():
    ...
    result = subprocess.run(["cache_demo.exe"], capture_output=True, text=True)
...
def create_plot_3_l1_border(sizes, seq_times, rand_times, title_suffix='Min Value', file_suffix='min', seq_err=None, rand_err=None):
    ...
    x_lo, x_hi = 16, 64
    mask = (sizes >= x_lo) & (sizes <= x_hi)
    visible = np.concatenate([seq_times[mask], rand_times[mask]])
    if seq_err is not None:
        visible = np.concatenate([visible, (seq_times + seq_err)[mask], (rand_times + rand_err)[mask]])
    y_max = visible.max() * 1.1 if len(visible) else 0.01
    plt.xlim(x_lo, x_hi)
    plt.ylim(0, y_max)
...
def create_plot_4_l2_border(sizes, seq_times, rand_times, title_suffix='Min Value', file_suffix='min', seq_err=None, rand_err=None):
    ...
    # Focus on L2 cache border region (1024KB to 4096KB)
    x_lo, x_hi = 1024, 4096
    mask = (sizes >= x_lo) & (sizes <= x_hi)
    visible = np.concatenate([seq_times[mask], rand_times[mask]])
    if seq_err is not None:
        visible = np.concatenate([visible, (seq_times + seq_err)[mask], (rand_times + rand_err)[mask]])
    y_max = visible.max() * 1.1 if len(visible) else 3.0
    plt.xlim(x_lo, x_hi)
    plt.ylim(0, y_max)
    ...
    plt.xlabel('Array Size (KB)', fontsize=12)
    plt.ylabel('Time (seconds)', fontsize=12)
    plt.title(f'Cache Performance Focus: L2 Cache Border (~2MB) - {title_suffix}', fontsize=14)
    plt.legend(fontsize=10)
    plt.grid(True, alpha=0.3, which="both", axis="both")
    plt.tight_layout()
